...
$content = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM cookie_policy WHERE id = 1");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id FROM cookie_policy WHERE id = 1");
        $stmt->execute();
        $exists = $stmt->fetch();

        if ($exists) {
            $sql = "UPDATE cookie_policy SET title = ?, content = ? WHERE id = 1";
        } else {
            $sql = "INSERT INTO cookie_policy (id, title, content) VALUES (1, ?, ?)";
        }
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$title, $page_content]);

        set_flash('success', 'Cookie Policy updated successfully.');
        header("Location: add-cookie-policy.php");
        exit;
    } catch (Exception $e) {
