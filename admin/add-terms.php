...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id FROM terms_and_conditions WHERE id = 1");
        $stmt->execute();
        $exists = $stmt->fetch();

        if ($exists) {
            $sql = "UPDATE terms_and_conditions SET title = ?, content = ? WHERE id = 1";
        } else {
            $sql = "INSERT INTO terms_and_conditions (id, title, content) VALUES (1, ?, ?)";
        }
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$title, $page_content]);

        set_flash('success', 'Terms & Conditions updated successfully.');
        header("Location: add-terms.php");
        exit;
    } catch (Exception $e) {
...
<head>
    <meta charset="UTF-8">
    <title>Admin: Edit Terms & Conditions</title>
...
<div class="container mx-auto p-8">
    <h1 class="text-3xl font-bold text-blue-900 mb-6">Edit Terms & Conditions</h1>

    <form action="add-terms.php" method="POST" class="bg-white p-8 rounded-lg shadow-md space-y-6">
        <div>
