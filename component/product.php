...
<div class="w-12 h-12 rounded-full overflow-hidden flex items-center justify-center">
    ...
    <?php else: ?>
        ...
        <?php if ($category === 'Facebook'): ?>
            <div class="w-full h-full bg-blue-600 flex items-center justify-center text-white font-bold">F</div>
        <?php elseif ($category === 'TikTok'): ?>
            <div class="w-full h-full bg-black flex items-center justify-center text-white font-bold">T</div>
        <?php elseif ($category === 'Twitter'): ?>
            <div class="w-full h-full bg-sky-500 flex items-center justify-center text-white font-bold">Tw</div>
        <?php else: ?>
            <div class="w-full h-full bg-gray-500 flex items-center justify-center text-white font-bold">?</div>
        <?php endif; ?>
    <?php endif; ?>
</div>
                        <div class="flex-1">
                            ...
                            <p class="text-gray-700 text-sm mt-1">Per Quantity: <span class="font-bold text-gray-900">₦<?= number_format($product['price'], 2) ?></span></p>
                        </div>
                    </div>
                    ...
                    <div class="flex-shrink-0">
                        <button @click="open = true" 
                                class="w-full md:w-auto px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 text-sm font-semibold flex items-center justify-center gap-2">
                            ✓ View account
                        </button>
                    </div>
                </div>
